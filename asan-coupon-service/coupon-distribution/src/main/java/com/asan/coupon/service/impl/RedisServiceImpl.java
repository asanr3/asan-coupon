...
public class RedisServiceImpl implements IRedisService {
    ...
    public void saveEmptyCouponListToCache(Long userId, List<Integer> status) {
        ...
        // key 是 coupon_id, value 是序列化的 Coupon  向map中放入无效的数据
        Map<String, String> invalidCouponMap = new HashMap<>();
        invalidCouponMap.put("-1", JSON.toJSONString(Coupon.invalidCoupon()));

        // 用户优惠券缓存信息
        // KV
        // K: status -> redisKey  根据不同的状态码得到一个redisKey
        // V: {Key:coupon_id  value:序列化的 Coupon}  redis的value是一个map结构
    ...
    private Integer addCouponToCacheForUsed(Long userId, List<Coupon> coupons)
        ...
        // 当前可用的优惠券个数一定是大于1的 因为本身已经存在一个无效的优惠券信息(用于避免缓存穿透) 加上现在的一个优惠券信息
        ...
        // isSubCollection(a,b)代表a是b的一个子集 ！代表不是一个子集
    ...
    private Integer addCouponToCacheForExpired(Long userId, List<Coupon> coupons)
        ...
        // 最终需要保存的 Cache 也就是过期的优惠券Expired
